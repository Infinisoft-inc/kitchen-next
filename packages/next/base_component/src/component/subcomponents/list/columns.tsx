...
import { IMicroStore } from "@/context/index"
import { TableConfig } from "component/types"
import { defaultAvatar } from "./assets/svg"


export const columns = (store: IMicroStore): TableConfig<Item> => {
  const onClick = (id: string) => {
    window.dispatchEvent(new CustomEvent('item.clicked'))
    store.mutate((prev: any) => ({ ...prev, editItemId: id }))
  }
  ...
  return {
    ...
    telephones: {
      render: (item: Item) => item?.telephones?.map((phone:any, i:number) => <div onClick={() => onClick(item.id)} key={phone} id={item.id}>{phone}</div>)
    },
